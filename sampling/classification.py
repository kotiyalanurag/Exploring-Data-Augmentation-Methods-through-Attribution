...
def get_stratified_subsets(df, importance_col="importance", label_col="label", frac=0.1, random_state=42):
    ...
    """
    top_subset = df.groupby(label_col, group_keys=False).apply(lambda x: x.nlargest(int(len(x) * frac), importance_col))
    bottom_subset = df.groupby(label_col, group_keys=False).apply(lambda x: x.nsmallest(int(len(x) * frac), importance_col))
